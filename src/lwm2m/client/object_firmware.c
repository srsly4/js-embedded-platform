...
static firmware_push_t firmware_push_buffer;
...
static uint8_t prv_firmware_write(uint16_t instanceId,
    ...
    i = 0;
    uint8_t *package_buff;
    do {
        switch (dataArray[i].id) {
            case RES_M_PACKAGE:
                // inline firmware binary
                if (firmware_push_buffer.code != NULL) {
                    lwm2m_free(firmware_push_buffer.code);
                    firmware_push_buffer.code;
                }
...
lwm2m_object_t *get_object_firmware(void) {
    firmware_push_buffer.size = 0;
    firmware_push_buffer.code = NULL;
    /*
